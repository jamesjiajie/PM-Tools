...
`dev` 模式会复用同一个端口：修改 `public/` 下的前端文件会自动刷新页面；修改 `server.js` 时 Node 会自动重启服务，浏览器重连后刷新。
...
- 左侧菜单多页面切换
- 历史项目页面集中查看所有项目
- 甘特图页面快速切换项目，并记住上次打开的项目
- 点击历史项目进入对应项目甘特图
- 项目名称编辑和保存
- 任务新增、编辑、删除
- 子任务关联到父任务，并在甘特图中缩进展示和跟踪
- 开发模式热更新，避免每次修改都换端口
- 隐藏周六日和 2026 年中国法定节假日
- 任务进度和状态快速更新
- 搜索、状态筛选、关键路径筛选
- 风险任务、里程碑、今天标记
- 本地 SQLite 持久化保存
- V3 数据库 schema version 记录
- V3 数据库升级前自动备份
...
V3 会记录当前数据库 schema version。服务启动时如果检测到数据库需要升级，会先执行 SQLite WAL checkpoint，再使用 `VACUUM INTO` 生成一致性备份，最后执行迁移。

服务启动完成后也会执行一次 WAL checkpoint，确保近期提交会同步回 `data/pm-tools.sqlite` 主库文件，降低只查看或备份主库文件时误以为记录丢失的风险。
